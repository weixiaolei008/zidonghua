...
<script>
    export default {
      ...
      methods: {
        runExecute(){
          var _this = this;
          var url = 'project/' + this.projectCode + '/testStress/execute'
          var p = { "id": this.$route.query.id }
          if(this.$route.query.sceneId != null){
            url = 'project/' + this.projectCode + '/testStressScene/execute'
            p = { "sceneId": this.$route.query.sceneId }
          }
          _this.$api.post(url, p, r => {
            ...
            if(r.data.projectCode!=null && r.data.status!=null){
              ...
              if(r.data.status=='405001'){
                alertMsg = r.data.projectCode+'项目'+'正在普通压测中，已运行'+r.data.exeTime+'秒，请稍后重试。';
              }
              if(r.data.status=='405002'){
                alertMsg = r.data.projectCode+'项目'+'正在功能测试中，已运行'+r.data.exeTime+'秒，不可同时进行，请稍后重试。';
              }
        ...
        openSocket() {
          ...
          if( userid==null && this.$route.query.sceneId != null){
            userid= this.$route.query.sceneId;
          }
          ...
          _this.stompClient.connect(
            ...
            function connectCallback(frame) {
              ...
              // }
                ...
                document.getElementById('log1').innerHTML += data.message + '<br/>';
                // console.log(data)
                if(data.status == 405001){
                   alert(123);
                }
                for (var i = 0;i < 6;i++) {
